...
# Copyright (c) 2022 Irdeto Canada Corporation.
#
#  All Rights Reserved.
#
...
DEREFERENCED_SOURCE_FILE=$(readlink "$SOURCE_FILE") 
IMPORT_PATH="$(dirname "${DEREFERENCED_SOURCE_FILE:-${SOURCE_FILE}}")/../import_example.sh" 
# shellcheck disable=SC1090
source "$IMPORT_PATH"
# Check if the 'sed' command calls GNU linux
# (The sed scripts in this script are not POSIX compliant
# and require GNU sed)
if ! sed --version 2>/dev/null | grep -q 'GNU'
then
  color_echo "red" "The sed command in this shell does not call GNU sed" 
  color_echo "red" "You can install GNU sed with homebrew:" 
  echo -ne "\t";  color_echo "green" "brew install gnu-sed"
  color_echo "red" "Then you can add the following line to your .bash_profile and .bashrc file:"
  echo -ne "\t"
  # shellcheck disable=SC2016
  color_echo "green" 'PATH="/usr/local/opt/gnu-sed/libexec/gnubin:$PATH'
  exit 1
fi

# Set the default server to the value of the GITLAB_DEFAULT_SERVER env variable
DEFAULT_SERVER=${GITLAB_DEFAULT_SERVER:-"gitlab.com"}
...
function create_gitlab_group_deployment_token {
    ...
    GITLAB_DEPLOYMENT_TOKEN="$(curl --silent --show-error --request POST --header "PRIVATE-TOKEN: ${4}" --header "Content-Type: application/json" \
    ...
    export GITLAB_DEPLOYMENT_TOKEN
...
function create_gitlab_project_api_token {
    ...
    token_expiration_date="${5:-${TOKEN_EXPIRATION_DATE:-$(date +%F --date="+3650 days")}}" # Default token lifetime is 10 years
    GITLAB_API_TOKEN="$(curl --silent --show-error --request POST --header "PRIVATE-TOKEN: ${4}" \
        --header "Content-Type:application/json" \
...
#######################################
# Retrieves the Runner token for a project and exports an environment variable called
# GITLAB_RUNNER_TOKEN with the token value.
# Arguments:
#   Name or ID of the project
#   User token to use for Gitlab authentication
#   (optional) Hostname of Gitlab server. If not set, the value of DEFAULT_GITLAB_SERVER will be used
#######################################
function get_gitlab_project_runner_token {
    # set the server that will be be used
    local server="${3:-${DEFAULT_SERVER}}"

    # exit if gitlab api token is not provided
    if [ -z "$2" ]; then
      color_echo "red" "Gitlab token not provided"
      exit 1
    fi
    
    
    # If the project isn't an integer look up it's ID

    if [[ ${1} == ?(-)+([0-9]) ]] ; then         
        project_id="${1}"
    else
        response="$(curl --silent --show-error --header "PRIVATE-TOKEN: ${2}" "https://${server}/api/v4/projects?search=${1}")"
        project_id="$( echo "${response}" | jq '.[] | select (.path == "'"${1}"'") | .id' || exit_on_fail "Failed to process response to Project lookup on server ${server}. HTTP response: $(echo "${response}" | jq . )")"
        echo "project_id: $project_id"
    fi

    debug 2 "Retrieving Gitlab runner registration token for project ${project_id} from server ${server}"
    GITLAB_RUNNER_TOKEN="$(
        curl \
          --silent \
          --show-error \
        ...
        | jq --raw-output ' .runners_token ' \
        || exit_on_fail "Failed to retrieve Gitlab Runner token for project ${1}, exiting!"\
    )"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]] ; then
    ...
    if [[ "${#:-0}" -ge 2 ]] && in_array "${1}" "${functions_delta[@]}" ; then
        ...
        "${@}"
    elif [[ "${#:-0}" -ge 1 ]] && in_array "${1}" "${help_flags[@]}" ; then
        print_usage 
        exit 0
    else
        print_usage
